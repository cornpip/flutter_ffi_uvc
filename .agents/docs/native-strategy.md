...
- Lifecycle goes through the request queue in `src/common/uvc_requests.cpp`, shared by every backend. A backend implements only the synchronous calls it is built on and calls `uvc_requests_shutdown` first thing in `uvc_session_destroy`. Keep verification and auto-selection native, and keep the Dart layer free of queues, epochs, and worker isolates.
