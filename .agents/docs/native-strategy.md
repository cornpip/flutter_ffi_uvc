...
- Every ABI call takes a `uvc_session_t*`. No backend may keep mutable per-camera state in file-scope globals. Process-level state is limited to the log level, device enumeration tables, framework startup flags, and the session registry behind `uvc_session_acquire`.
